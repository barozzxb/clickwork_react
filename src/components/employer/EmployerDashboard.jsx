...
import { FaPlus, FaEdit, FaLock, FaLockOpen, FaUserFriends, FaBriefcase, FaMapMarkerAlt, FaTag, FaCalendarAlt, FaDollarSign } from 'react-icons/fa';
...
const EmployerDashboard = () => {
    ...
    const fetchJobs = async () => {
        try {
            ...
            if (response.data.status) {  // Sử dụng status thay vì success theo API
                ...
                {loading && jobs.length === 0 ? (
                    ...
                    <>
                            ...
                            <div className="card-body">
                                ...
                                <div className="row g-3">
                                    <div className="col-md-6">
                                        <div className="mb-2">Loại công việc:</div>
                                        <div className="d-flex flex-wrap gap-2">
                                            <button
                                                className={`btn btn-sm ${jobTypeFilters.ALL ? 'btn-primary' : 'btn-outline-primary'}`}
                                                onClick={() => handleJobTypeFilterChange('ALL')}
                                            >
                                                Tất cả
                                            </button>
                                            <button
                                                className={`btn btn-sm ${jobTypeFilters.FULLTIME ? 'btn-primary' : 'btn-outline-primary'}`}
                                                onClick={() => handleJobTypeFilterChange('FULLTIME')}
                                            >
                                                Toàn thời gian
                                            </button>
                                            <button
                                                className={`btn btn-sm ${jobTypeFilters.PARTTIME ? 'btn-primary' : 'btn-outline-primary'}`}
                                                onClick={() => handleJobTypeFilterChange('PARTTIME')}
                                            >
                                                Bán thời gian
                                            </button>
                                            <button
                                                className={`btn btn-sm ${jobTypeFilters.INTERNSHIP ? 'btn-primary' : 'btn-outline-primary'}`}
                                                onClick={() => handleJobTypeFilterChange('INTERNSHIP')}
                                            >
                                                Thực tập
                                            </button>
                                            <button
                                                className={`btn btn-sm ${jobTypeFilters.FREELANCE ? 'btn-primary' : 'btn-outline-primary'}`}
                                                onClick={() => handleJobTypeFilterChange('FREELANCE')}
                                            >
                                                Tự do
                                            </button>
                                            <button
                                                className={`btn btn-sm ${jobTypeFilters.FLEXIBLE ? 'btn-primary' : 'btn-outline-primary'}`}
                        ...
                        <div className="mb-3 d-flex justify-content-between align-items-center">
                            <p className="mb-0">Đã tìm thấy {filteredJobs.length} công việc</p>
                        ...
                        <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
                            {filteredJobs.map((job) => (
                                <div key={job.id} className="col">
                                    <div className={`card h-100 ${!job.active ? 'border-secondary' : ''}`}>
                                        <div className={`card-header d-flex justify-content-between align-items-center ${!job.active ? 'bg-light text-muted' : ''}`}>
                                            <span className={`badge ${job.active ? 'bg-success' : 'bg-secondary'}`}>
                                                {job.active ? 'Đang hiển thị' : 'Đã ẩn'}
                                            </span>
                                            <div>
                                                <button
                                                    className={`btn btn-sm ${job.active ? 'btn-outline-secondary' : 'btn-outline-success'} me-1`}
                                                    onClick={() => handleToggleStatus(job.id)}
                                                    title={job.active ? "Ẩn công việc" : "Hiển thị công việc"}
                                                    disabled={loading}
                                                >
                                        ...
                                        <div className={`card-body ${!job.active ? 'bg-light text-muted' : ''}`}>
                                            ...
                                            <div className="mb-3">
                                                ...
                                                <div className="d-flex align-items-center mb-1">
                                                    ...
                                                    <span>
                                                        {job.createdat ? new Date(job.createdat).toLocaleDateString('vi-VN') : 'Không xác định'}
                                                    </span>
                                                </div>
                                            </div>

                                            <div className="mb-2">
                                                <div><strong>Lĩnh vực:</strong> {job.field || 'Không xác định'}</div>
                                                <div><strong>Số lượng:</strong> {job.quantity || 'Không xác định'}</div>
                                            </div>
                                        ...
                                        <div className={`card-footer bg-transparent ${!job.active ? 'text-muted' : ''}`}>
                                            <div className="d-flex justify-content-between">
                                                <Link to={`/employer/jobs/${job.id}`} className="btn btn-sm btn-outline-primary">
                                                    Chi tiết
                                                </Link>
                                                <div>
                                                    <Link to={`/employer/jobs/edit/${job.id}`} className="btn btn-sm btn-outline-warning me-1">
                                                        <FaEdit /> Sửa
                                                    </Link>
                                                    <Link to={`/employer/jobs/${job.id}/applicants`} className="btn btn-sm btn-outline-info">
