...
import { FaHeadset, FaFlag, FaBell, FaUser, FaCog, FaBriefcase } from 'react-icons/fa';
...
const EmployerNavbar = () => {
    ...
    const fetchNotifications = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:9000/api/employer/profile/notifications', {
    ...
    const handleMarkAsRead = async (id) => {
        try {
            setLoading(true);
            const token = localStorage.getItem('token');
            await axios.patch(`http://localhost:9000/api/employer/profile/notifications/${id}/read`, {}, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            ...
            <div className="container-fluid">
                ...
                <div className="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul className="navbar-nav me-auto mb-2 mb-lg-0">
                        ...
                        <li className="nav-item">
                            <Link className="nav-link" to="/employer">Quản lý công việc</Link>
                        </li>
                    </ul>
                    ...
                    <div className="ms-auto">
                        <ul className="navbar-nav d-flex align-items-center">
                            {/* Notifications Icon */}
                            <li className="nav-item me-3 position-relative" ref={notificationRef}>
                                <div
                                    className="nav-link"
                                        ...
                                        {notifications.length === 0 ? (
                                            ...
                                            notifications.slice(0, 5).map(notification => (
                                                ...
                                                >
                                                    ...
                                                    <p className="mb-0 text-truncate">{notification.content}</p>
                                                </div>
                                            ))
                                        )}
                                    </div>
